...
public class GetBookingsHandler : IRequestHandler<GetBookingsQuery, GetBookingsResult>
{
    ...
    public async Task<GetBookingsResult> Handle(GetBookingsQuery query, CancellationToken cancellationToken)
    {
        ...
        else if (query.Role == "CourtOwner")
        {
            ...
            var ownedSportsCenterIds = ownedSportsCenters.Select(sc => sc.Id.Value).ToList();
            bookingsQuery = bookingsQuery.Where(b => b.BookingDetails.Any(d =>
                _context.Courts.Any(c => c.Id.Value == d.CourtId.Value && ownedSportsCenterIds.Contains(c.SportCenterId.Value))));
            ...
            if (query.SportsCenterId.HasValue)
            {
                if (!ownedSportsCenterIds.Contains(query.SportsCenterId.Value))
        ...
        var allCourtIds = bookings.SelectMany(b => b.BookingDetails.Select(d => d.CourtId.Value)).Distinct().ToList();
        var courts = await _context.Courts
            .Where(c => allCourtIds.Contains(c.Id.Value))
            .ToDictionaryAsync(c => c.Id.Value, c => c, cancellationToken);

        var sportCenterIds = courts.Values.Select(c => c.SportCenterId.Value).Distinct().ToList();
        ...
        var bookingDtos = bookings.Select(b => new BookingDto(
            ...
            {
                var court = courts[d.CourtId.Value];
                var sportCenterName = sportCenters[court.SportCenterId.Value];
